...
class EpisodesWindow(kodigui.ControlledWindow, windowutils.UtilMixin):
    ...
    def infoButtonClicked(self):
        ...
        opener.handleOpen(
            ...
            sub_title=subtitle,
            thumb=episode.thumb,
            thumb_fallback='script.plexodus/thumb_fallbacks/show.png',
    ...
    def setItemInfo(self, video, mli):
        ...
        if video.get('ratingImage'):
            ...
            mli.setProperty('rating.image', 'script.plexodus/ratings/{0}.png'.format(video.ratingImage.replace('://', '/')))
            if video.get('audienceRatingImage'):
                mli.setProperty('rating2', audienceRating)
                mli.setProperty('rating2.image', 'script.plexodus/ratings/{0}.png'.format(video.audienceRatingImage.replace('://', '/')))
        else:
    ...
    def fillEpisodes(self, update=False):
        ...
        self.episodeListControl.addItems(
            [kodigui.ManagedListItem(properties={'thumb.fallback': 'script.plexodus/thumb_fallbacks/show.png'}) for x in range(len(episodes))]
        )
    ...
    def fillExtras(self):
        ...
        for extra in self.season.extras():
            ...
            if mli:
                mli.setProperty('index', str(idx))
                mli.setProperty(
                    'thumb.fallback', 'script.plexodus/thumb_fallbacks/{0}.png'.format(extra.type in ('show', 'season', 'episode') and 'show' or 'movie')
                )
                items.append(mli)
                idx += 1
