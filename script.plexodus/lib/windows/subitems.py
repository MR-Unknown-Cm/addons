...
class ShowWindow(kodigui.ControlledWindow, windowutils.UtilMixin):
    ...
    def updateProperties(self):
        ...
        if self.mediaItem.ratingImage:
            ...
            self.setProperty('rating.image', 'script.plexodus/ratings/{0}.png'.format(self.mediaItem.ratingImage.replace('://', '/')))
            if self.mediaItem.audienceRatingImage:
                self.setProperty('rating2', audienceRating)
                self.setProperty('rating2.image', 'script.plexodus/ratings/{0}.png'.format(self.mediaItem.audienceRatingImage.replace('://', '/')))
        else:
    ...
    def infoButtonClicked(self):
        fallback = 'script.plexodus/thumb_fallbacks/{0}.png'.format(self.mediaItem.type == 'show' and 'show' or 'music')
