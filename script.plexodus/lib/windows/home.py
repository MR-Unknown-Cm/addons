...
class HomeWindow(kodigui.BaseWindow, util.CronReceiver):
    ...
    def displayServerAndUser(self, **kwargs):
        ...
        if plexapp.SERVERMANAGER.selectedServer:
            ...
            self.setProperty('server.icon', 'script.plexodus/home/device/plex.png')  # TODO: Set dynamically to whatever it should be if that's how it even works :)
            self.setProperty('server.iconmod', plexapp.SERVERMANAGER.selectedServer.isSecure and 'script.plexodus/home/device/lock.png' or '')
        else:
            self.setProperty('server.name', T(32338, 'No Servers Found'))
            self.setProperty('server.icon', 'script.plexodus/home/device/error.png')
    ...
    def showSections(self):
        ...
        if pl:
            plli = kodigui.ManagedListItem('Playlists', thumbnailImage='script.plexodus/home/type/playlists.png', data_source=PlaylistsSection)
        ...
        for section in sections:
            mli = kodigui.ManagedListItem(section.title, thumbnailImage='script.plexodus/home/type/{0}.png'.format(section.type), data_source=section)
    ...
    def createClipListItem(self, obj, wide=False):
        ...
        mli.setProperty('thumb.fallback', 'script.plexodus/thumb_fallbacks/movie16x9.png')
        return mli
    ...
    def createPlaylistListItem(self, obj, wide=False):
        ...
        mli.setProperty('thumb.fallback', 'script.plexodus/thumb_fallbacks/{0}.png'.format(obj.playlistType == 'audio' and 'music' or 'movie'))
        return mli
