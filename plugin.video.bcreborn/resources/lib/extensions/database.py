...
class Database(object):
	...
	def __init__(self, name, addon = None, default = None, path = None, connect = True):
		try:
			...
			xbmc.log('Bone Crusher Reborn ERROR [Database]: ' + str(error), xbmc.LOGERROR)
	...
	def clear(self, tables = None, confirm = False):
		...
		if not confirm or xbmcgui.Dialog().yesno(title, message):
			...
			if confirm:
				...
				icon = xbmc.translatePath(xbmcaddon.Addon('script.bcreborn.resources').getAddonInfo('path').decode('utf-8'))
