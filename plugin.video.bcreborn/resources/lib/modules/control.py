...
def artPath():
    theme = appearance()
    return os.path.join(xbmcaddon.Addon('plugin.video.bcreborn').getAddonInfo('path'), 'resources', 'artwork', theme)
#    if theme in ['-', '']: return
...
def artwork():
    if condVisibility('System.HasAddon(script.bcreborn.artwork)'):
        execute('RunPlugin(plugin://script.bcreborn.artwork)')
