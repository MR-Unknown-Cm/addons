...
V2_API_KEY = 'a8503d6c335d1b88d55e78dd6fdf7d481dac86b22263f5e8aaf56362a5f756f6'
CLIENT_SECRET = 'cb628f9770c262272f8b17cc8ecbdb491bba46faea71da9d0948b185552fba43'
...
def _cache(url, post = None, timestamp = None):
    try:
        ...
        post = ('"%s"' % post.replace('"', '""').replace("'", "''")) if not post is None else data._null()
        ...
        if select >= 0:
            if select == 0:
                ...
                if select == 8:
                    ...
                    if not slug is None: getTrakt(items[select][1] % slug, post = post)[0]
                else:
...
def scrobbleProgress(type, imdb = None, tvdb = None, season = None, episode = None):
    try:
        ...
        if not imdb is None: imdb = str(imdb)
        if not tvdb is None: tvdb = int(tvdb)
        if not episode is None: episode = int(episode)
        if not episode is None: episode = int(episode)
            ...
            if len(items) > 0:
                ...
                if item:
                    ...
                    data = {
                        type : item,
                        'progress' : progress,
                        'app_version' : control.addonVersion(addon='plugin.video.bcreborn'),
                    }
