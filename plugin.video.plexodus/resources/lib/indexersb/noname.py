...
#please dont share link
    ...
    def root(self):
        try:
            regex.clear()
            url = 'http://localhost/xmls/main.xml'
            self.list = self.plexOdus_list(url)
    ...
    def getx(self, url, worker=False):
        try:
            ...
            url = regex.resolve(r)
            self.list = self.plexOdus_list('', result=url)
            self.addDirectory(self.list)
            return self.list
        except:
            pass
    ...
    def developer(self):
        try:
            ...
            self.list = self.plexOdus_list('', result=result)
    ...
    def addSearch(self, url):
            ...
            self.list = [] ; threads = [] 
            for link in links: threads.append(workers.Thread(self.plexOdus_list, link))
    ...
    def plexOdus_list(self, url, result=None):
