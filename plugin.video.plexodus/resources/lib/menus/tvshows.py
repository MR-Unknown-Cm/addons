...
class TVshows:
    ...
    def search(self):
        navigator.Navigator().addDirectoryItem(32603, 'tvSearchnew', 'icon.png', 'icon.png')
        try:
        ...
        for (id,term) in dbcur.fetchall():
            if term not in str(lst):
                delete_option = True
                navigator.Navigator().addDirectoryItem(term, 'tvSearchterm&name=%s' % term, 'icon.png', 'icon.png')
                lst += [(term)]
        dbcur.close()
        if delete_option:
            navigator.Navigator().addDirectoryItem(32605, 'clearCacheSearch', 'icon.png', 'DefaultAddonProgram.png')
    ...
    def genres(self):
        ...
        for i in genres:
            self.list.append({'name': cleangenre.lang(i[0], self.lang), 'url': self.genre_link % i[1] if i[2] else self.keyword_link % i[1], 'image': 'icon.png', 'action': 'tvshows'})
    ...
    def languages(self):
        ...
        for i in languages:
            self.list.append({'name': str(i[0]), 'url': self.language_link % i[1], 'image': 'icon.png', 'action': 'tvshows'})
        self.addDirectory(self.list)
        return self.list
    ...
    def certifications(self):
        ...
        for i in certificates: self.list.append({'name': str(i[0]), 'url': self.certification_link % self.certificatesFormat(i[1]), 'image': 'icon.png', 'action': 'tvshows'})
        self.addDirectory(self.list)
        return self.list
    ...
    def userlists(self):
        ...
        try:
            ...
            for i in range(len(lists)): lists[i].update({'image': 'icon.png'})
            userlists += lists
        except: pass
        ...
        if self.imdb_user != '':
            ...
            self.list.insert(0, {'name': control.lang(32033).encode('utf-8'), 'url': imdb_watchlist, 'image': 'icon.png', 'action': 'tvshows'})

        # Trakt Watchlist
        if trakt.getTraktCredentialsInfo():
            trakt_watchlist = self.traktwatchlist_link + 'shows'
            self.list.insert(0, {'name': control.lang(32033).encode('utf-8'), 'url': trakt_watchlist, 'image': 'icon.png', 'action': 'tvshows'})
