...
class Episodes:
    ...
    def calendars(self, idx=True):
        ...
        for i in range(0, 30):
            try:
                ...
                self.list.append({'name': name, 'url': url, 'image': 'icon.png', 'action': 'calendar'})
            except:
                pass
    ...
    def userlists(self):
        ...
        for i in range(0, len(self.list)): self.list[i].update({'image': 'icon.png', 'action': 'calendar'})

        # Watchlist
        if trakt.getTraktCredentialsInfo():
            self.list.insert(0, {'name': control.lang(32033).encode('utf-8'), 'url': self.traktwatchlist_link,
                                 'image': 'icon.png', 'action': 'tvshows'})
