...
class Movies:
    ...
    def search(self):
        navigator.Navigator().addDirectoryItem(32603, 'movieSearchnew', 'icon.png', 'icon.png')
        try:
        ...
        for (id, term) in dbcur.fetchall():
            if term not in str(lst):
                delete_option = True
                navigator.Navigator().addDirectoryItem(term, 'movieSearchterm&name=%s' % term, 'icon.png', 'icon.png')
                lst += [(term)]
        dbcur.close()
        if delete_option:
    ...
    def genres(self):
        ...
        for i in genres:
            self.list.append({'name': cleangenre.lang(i[0], self.lang), 'url': self.genre_link % i[1] if i[2] else self.keyword_link % i[1], 'image': 'icon.png', 'action': 'movies' })
    ...
    def languages(self):
        ...
        for i in languages:
            self.list.append({'name': str(i[0]), 'url': self.language_link % i[1], 'image': 'icon.png', 'action': 'movies'})
        self.addDirectory(self.list)
        return self.list
    ...
    def certifications(self):
        ...
        for i in certificates: self.list.append({'name': str(i[0]), 'url': self.certification_link % self.certificatesFormat(i[1]), 'image': 'icon.png', 'action': 'movies'})
        self.addDirectory(self.list)
        return self.list
    ...
    def userlists(self):
        ...
        if trakt.getTraktCredentialsInfo():
            self.list.insert(0, {'name': control.lang(32033).encode('utf-8'), 'url' : self.traktwatchlist_link, 'image': 'icon.png', 'action': 'movies'})
