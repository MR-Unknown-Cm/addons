...
def artPath():
    theme = appearance()
    return os.path.join(xbmcaddon.Addon('plugin.video.plexodus').getAddonInfo('path'), 'resources', 'artwork', theme)
#    if theme in ['-', '']: return
...
def artwork():
    if condVisibility('System.HasAddon(script.plexOdus.artwork)'):
        execute('RunPlugin(plugin://script.plexOdus.artwork)')
