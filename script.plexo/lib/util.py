...
def LOG(msg, level=xbmc.LOGNOTICE):
    xbmc.log('script.plexo: {0}'.format(msg), level)
...
def setGlobalProperty(key, val):
    xbmcgui.Window(10000).setProperty('script.plexo.{0}'.format(key), val)


def setGlobalBoolProperty(key, boolean):
    xbmcgui.Window(10000).setProperty('script.plexo.{0}'.format(key), boolean and '1' or '')


def getGlobalProperty(key):
    return xbmc.getInfoLabel('Window(10000).Property(script.plexo.{0})'.format(key))
...
def getProgressImage(obj):
    ...
    return 'script.plexo/progress/{0}.png'.format(pct)
