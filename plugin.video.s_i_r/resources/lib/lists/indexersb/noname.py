...
class indexer:
    ...
    def root(self):
        try:
            regex.clear()
            url = 'https://raw.githubusercontent.com/Firegod36/apocalypse/master/watchdogs.xml'
    ...
    def youtube(self, url, action):
        try:
            ...
            if 'Tuner' in action:
                ...
                for i in self.list: i.update({'name': i['title'], 'poster': i['image'], 'nextaction': action, 'action': 'play1', 'folder': False})
                self.addDirectory(self.list)
