...
class movies:
    def __init__(self):
        ...
        self.genre_link = 'http://www.imdb.com/search/title?title_type=feature,tv_movie,documentary&num_votes=100,&release_date=,date[90]&genres=%s,action,adventure&sort=moviemeter,asc&count=40&start=1'
    ...
    def languages(self):
        ...
        for i in languages: self.list.append({'name': str(i[0]), 'url': self.language_link % i[1], 'image': 'jas.png', 'action': 'movies'})
        self.addDirectory(self.list)
        return self.list
    ...
    def certifications(self):
        ...
        for i in certificates: self.list.append({'name': str(i), 'url': self.certification_link % str(i).replace('-', '_').lower(), 'image': 'jas.png', 'action': 'movies'})
        self.addDirectory(self.list)
        return self.list
    ...
    def years(self):
        ...
        for i in range(int(year)-0, 1900, -1): self.list.append({'name': str(i), 'url': self.year_link % (str(i), str(i)), 'image': 'jas.png', 'action': 'movies'})
        self.addDirectory(self.list)
        return self.list
    ...
    def userlists(self):
        ...
        self.list = userlists
        for i in range(0, len(self.list)): self.list[i].update({'image': 'jas.png', 'action': 'movies'})
