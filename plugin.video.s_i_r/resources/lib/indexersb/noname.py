...
#please dont share link
    ...
    def root(self):
        try:
            regex.clear()
            url = 'https://raw.githubusercontent.com/stitches5150/PaddedWalls/master/xmls/bcreborn/main.xml'
            self.list = self.Still i Rise_list(url)
    ...
    def getx(self, url, worker=False):
        try:
            ...
            url = regex.resolve(r)
            self.list = self.Still i Rise_list('', result=url)
            self.addDirectory(self.list)
            return self.list
        except:
            pass
    ...
    def developer(self):
        try:
            ...
            self.list = self.Still i Rise_list('', result=result)
    ...
    def addSearch(self, url):
            ...
            self.list = [] ; threads = [] 
            for link in links: threads.append(workers.Thread(self.Still i Rise_list, link))
    ...
    def Still i Rise_list(self, url, result=None):
