...
def add_tvshow_to_library(library_folder, show, play_plugin = None):
    ...
    if not xbmcvfs.exists(playlist_folder):
        ...
        except: dialogs.notify(msg=_('Creation of Plexodus player Playlist Folder'), title=_('Failed'), delay=5000, image=get_icon_path("lists"))
...
#    elif xbmc.getCondVisibility("system.hasaddon(script.lamentbox)"): xbmc.executebuiltin("RunScript(script.lamentbox,info=afteradd)")
    return clean_needed
...
def setup_library(library_folder):
    ...
    if not xbmcvfs.exists(library_folder):
        ...
        msg = _("Would you like to automatically set Plexodus player as a tv shows source?")
        if dialogs.yesno("{0} {1}".format(_("Library"), "setup"), msg):
            try:
                source_thumbnail = get_icon_path("tv")
                source_name = "Plexodus player " + _("TV shows")
