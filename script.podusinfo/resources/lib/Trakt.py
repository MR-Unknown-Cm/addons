...
TRAKT_KEY = 'a8503d6c335d1b88d55e78dd6fdf7d481dac86b22263f5e8aaf56362a5f756f6'
...
def get_trakt_calendar_shows(content):
    ...
    url = ""
    if content == "shows":
        url = 'calendars/new-shows/%s/1?extended=full,images' % str(date.today() - timedelta(days=2))
        ...
        for episode in day[1]:
            if episode["show"]["ids"]["tvdb"] or episode["show"]["ids"]["tmdb"] or episode["show"]["ids"]["imdb"] or episode["show"]["ids"]["tvrage"]:
                ...
                show = {'title': "%s - %dx%d - %s" % (episode["show"]["title"], episode["episode"]["season"], episode["episode"]["number"], episode["episode"]["title"]),
                        ...
                        'path': 'plugin://script.podusinfo/?info=extendedtvinfo&tvdb_id=%s' % episode["show"]["ids"]["tvdb"],
...
def handle_trakt_movies(results):
    ...
    for movie in results:
        ...
        movie = {'title': movie["movie"]["title"],
                 ...
                 'Trailer': 'plugin://script.podusinfo/?info=playtrailer&id=%s' % str(movie["movie"]["ids"]["tmdb"]),
                 'year': movie["movie"]["year"],
                 'id': id,
                 'imdb_id': movie["movie"]["ids"]["imdb"],
                 'trakt_id': movie["movie"]["ids"]["trakt"],
                 'slug': movie["movie"]["ids"]["slug"],
                 'path': 'plugin://script.podusinfo/?info=podusinfo&id=%s' % str(movie["movie"]["ids"]["tmdb"]), 
...
def handle_trakt_tvshows(results):
    ...
    for tvshow in results:
        ...
        show = {'title': tvshow['show']["title"],
                ...
                'Trailer': 'plugin://script.podusinfo/?info=playtvtrailer&id=%s' % str(tvshow['show']['ids']["tmdb"]),
                ...
                'path': 'plugin://script.podusinfo/?info=extendedtvinfo&id=%s' % str(tvshow['show']['ids']["tmdb"]),
