...
class DialogBaseInfo(object):
    ...
    def onInit(self, *args, **kwargs):
        ...
        HOME.setProperty("podusinfo_fanart", self.info.get("fanart", ""))
            ...
            if selection == 0:
                if self.type == "TVShow":
                    if dbid:
                        ...
                        xbmc.executebuiltin("ActivateWindow(10025,plugin://plugin.video.plexodusplayer/tv/tvdb/%s,return)" % tvdb)
                        self.close()
                else:
                    if dbid: url = "temp"
                    else:
                        dbid = 0
                        url = "plugin://plugin.video.plexodusplayer/movies/play/tmdb/%s/%s" % (item_id, SETTING("player_main_movie"))
                    PLAYER.qlickplay(url, listitem=None, window=self, dbid=dbid)
            if selection == 1:
                if self.type == "TVShow":
                    if dbid:
                        show_path = xbmc.translatePath("special://profile/addon_data/plugin.video.plexodusplayer/TVShows/%s" % tvdb)
                        if os.path.exists(show_path):
                            try:
                                shutil.rmtree(show_path)
                                xbmc.executebuiltin("RunScript(script.podusinfo,info=afteradd)")
                    ...
                    if dbid:
                        movie_path = xbmc.translatePath("special://profile/addon_data/plugin.video.plexodusplayer/Movies/%s" % imdb)
